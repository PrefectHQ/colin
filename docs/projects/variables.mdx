...
Add variables to the `[vars]` section of your `colin.toml`.
...
Date and timestamp types parse ISO 8601 formatted strings. The `secret` type wraps values in Pydantic's `SecretStr`, which redacts the value in logs and string representations.
