...
The `colin clean` command removes stale files from `output/`:

```bash
colin clean
```

Stale files are outputs no longer tracked by the manifest—they appear when you delete or rename model files. By default, `clean` only removes stale files from `output/`, leaving your cache intact.

Clean prompts for confirmation before removing files:
...
```
Warning: 2 stale files in output/. Run `colin clean` to remove.
```

For recompiling without removing stale files, use `colin run --no-cache` instead.
