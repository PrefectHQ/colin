...
```bash
colin init --name my-context --models src/docs --target dist
```
...
| `--target` | Compiled output directory | `target` |
...
colin run --target ./out  # Override output directory
```
...
Will remove:
  ...
  target/compiled/company.md
  target/compiled/products/overview.md

Continue? (y/N)
```

Type `y` to proceed. The entire `target/` directory is deleted, including the manifest and all compiled documents.
