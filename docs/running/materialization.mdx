...
The `manifest.json` file in your `target/` directory tracks all compilation state. It records:
...
The manifest writes to `target/manifest.json` by default. If you override the target directory with `--target`, the manifest goes there instead.

## Force Recompilation

Sometimes you want fresh results regardless of what changed. The `--force` flag discards the manifest and recompiles everything:
...
Force recompilation incurs the full cost of all LLM calls since nothing is cached.

## Cleaning

The `colin clean` command removes the entire `target/` directory, including the manifest:
