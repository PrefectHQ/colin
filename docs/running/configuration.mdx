...
MCP servers are configured under `[[providers.mcp]]`. Each entry requires a `name` that becomes accessible in templates as `colin.mcp.<name>`. See [Providers](/context/providers) for usage documentation.
