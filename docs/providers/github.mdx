...
Use `ref()` to track GitHub resources for staleness detection:

```jinja
{# Fetched but not tracked #}
{{ github.prefect.file("README.md") }}

{# Tracked — document recompiles when file changes #}
{{ ref(github.prefect.file("README.md")) }}
```

Track a directory listing to recompile when files are added or removed:
...
Track individual files from a listing:
