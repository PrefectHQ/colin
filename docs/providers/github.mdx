...
```jinja
{{ github.file("PrefectHQ/prefect", "README.md").content }}
{{ github.file("owner/repo", "src/main.py", ref="v1.0").content }}
```

For frequently accessed repositories, configure named instances to avoid repeating the repo:

```jinja
{{ github.prefect.file("README.md").content }}
```
...
{{ github.prefect.file("README.md").content }}
{{ github.prefect.file("src/prefect/flows.py", ref="main").content }}
```
...
```jinja
{# Default provider: repo as first argument #}
{% for entry in github.ls("PrefectHQ/prefect", "src/prefect/") %}
...
{{ ref(github.prefect.file("README.md")).content }}

{# Opt out of tracking with watch=False #}
{{ github.prefect.file("README.md", watch=False).content }}
```
...
{{ github.prefect.file(entry.path).content }}
{% endif %}
{% endfor %}
```
...
{{ github.prefect.file("docs/api/flows.md", ref="main").content }}
```
...
```python
{{ github.prefect.file("src/prefect/settings.py", ref="v3.0.0").content }}
```
```
...
{{ github.file("anthropics/claude-code", "LICENSE").content }}
```
