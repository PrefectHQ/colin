...
| :---- | :---------- | :------ |
| `model` | Model identifier (e.g., `anthropic:claude-sonnet-4-20250514`) | From settings |
...
The default provider's functions are available directly on the `llm` namespace:

```markdown
{{ llm.extract(content, 'summary') }}
{{ llm.classify(content, ['positive', 'negative']) }}
...
```markdown
{{ llm.fast.extract(content, 'word count') }}
{{ llm.capable.extract(content, 'detailed legal analysis') }}
```
...
The LLM provider exposes three functions. When using the default provider, these are available both as direct functions (`extract()`, `classify()`) and on the `llm` namespace (`llm.extract()`, `llm.classify()`).
...
```markdown
{{ llm.extract(content, 'summary', model='anthropic:claude-haiku-3') }}
```
...
```markdown
{{ ref('sources/report') | extract('the top 5 key findings') }}
```
...
## Summary
{{ ref('sources/report') | extract('a one-paragraph executive summary') }}

## Risks
{{ ref('sources/report') | extract('identified risks and mitigation strategies') }}

## Action Items
{{ ref('sources/report') | extract('immediate action items with owners') }}
```
...
```markdown
{% set sentiment = llm.classify(ref('feedback').content, ['positive', 'negative', 'neutral']) %}
...
```markdown
{% set urgent = llm.classify(ref('ticket').content, [true, false]) %}
...
```markdown
{% set tags = llm.classify(ref('article').content, ['technical', 'business', 'tutorial', 'news'], multi=true) %}
...
```markdown
{{ llm.complete('Write a haiku about documentation') }}
```
...
```markdown
{{ ref('data') | extract('word count', model='anthropic:claude-haiku-3') }}
```
...
{{ ref('sources/raw') | extract('structured summary') }}
```
