...
Colin includes five built-in providers:
...
Providers are configured in `colin.toml` under `[[providers.<type>]]` sections. The double brackets indicate a TOML array—you can define multiple instances of the same provider type.
...
```markdown
{{ llm.fast.extract(content, 'quick summary') }}
```
...
{{ llm.capable.extract(content, 'detailed analysis') }}
```
...
A provider with `schemes = ["http", "https"]` that returns `{"get": self._get}` becomes accessible as `http.get()` in templates. The first scheme in the list determines the namespace.
...
{{ http.get('https://api.example.com/data').content }}

{# Tracked as a dependency #}
{{ ref(http.get('https://api.example.com/data')).content }}
```
