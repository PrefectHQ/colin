...
2. The compiled output is stored in `target/`
...
Colin validates references at compile time. A reference to a non-existent document fails with a clear error:

```
RefNotFoundError: Referenced document not found: project://missing.md
```
