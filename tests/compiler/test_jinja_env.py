...
async def test_llm_extract_filter_generates_position_based_ids(tmp_path: Path) -> None:
    ...
    """
    captured_cache_ids: list[str | None] = []

    # Create a mock LLM namespace that captures _cache_id
    class MockLLMNamespace:
        async def extract(
            ...
            captured_cache_ids.append(_cache_id)
    ...
    await filter_func("content1", "prompt1")
    await filter_func("content2", "prompt2")
    await filter_func("content3", "prompt3")

    # Should generate sequential IDs
    assert captured_cache_ids == ["extract_1", "extract_2", "extract_3"]
...
async def test_llm_extract_filter_respects_explicit_cache_id(tmp_path: Path) -> None:
    ...
    await filter_func("content1", "prompt1")  # auto
    await filter_func("content2", "prompt2", _cache_id="custom_id")  # explicit
    await filter_func("content3", "prompt3")  # auto continues

    # Explicit ID used, auto counter continues
    assert captured_cache_ids == ["extract_1", "custom_id", "extract_3"]
